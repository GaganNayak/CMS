...
local_server= True
app = Flask(__name__)
app.secret_key='gagan'

app.config['MYSQL_HOST'] = 'localhost'
app.config['MYSQL_USER'] = 'root'
app.config['MYSQL_PASSWORD'] = 'db12'
app.config['MYSQL_DB'] = 'clinic1'
...
app.config.update(
    ...
    MAIL_USE_SSL=False,
    MAIL_USE_TSL = True,
    MAIL_USERNAME="8f3d51287d45d8",
    MAIL_PASSWORD='5373127cceda2f'
)
mail = Mail(app)
...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:db12@localhost/clinic1'
db=SQLAlchemy(app)
...
def doctors():
    ...
    if request.method=="POST":
        ...
        query=db.engine.execute(f"INSERT INTO `doctors` (`email`,`doctorname`) VALUES ('{email}','{doctorname}')")
...
def patient():
    ...
    if request.method=="POST":
        ...
        query=db.engine.execute(f"INSERT INTO `patients` (`email`,`name`,	`gender`,`slot`,`disease`,`time`,`date`,`number`) VALUES ('{email}','{name}','{gender}','{slot}','{disease}','{time}','{date}','{number}')")
...
def edit(pid):
    ...
    if request.method=="POST":
        ...
        date=request.form.get('date')
        number=request.form.get('number')
        db.engine.execute(f"UPDATE `patients` SET `email` = '{email}', `name` = '{name}', `gender` = '{gender}', `slot` = '{slot}', `disease` = '{disease}', `time` = '{time}', `date` = '{date}', `number` = '{number}' WHERE `patients`.`pid` = {pid}")
